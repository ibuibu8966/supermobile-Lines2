...
// Machinegun用のページ設定を上書き
const machinegunAuthConfig: NextAuthConfig = {
  ...authConfig,
  pages: {
    signIn: "/login",
    error: "/login",
  ...
  callbacks: {
    ...
    async authorized({ auth, request: { nextUrl } }: AuthorizedParams) {
      ...
      // 公開ページ（認証不要）
      const publicPaths = ["/", "/apply", "/login", "/register", "/forgot-password", "/legal", "/privacy", "/terms"];
      const isPublicPage = publicPaths.some(
        (path) => pathname === path || pathname.startsWith("/apply/") || pathname.startsWith("/documents/")
      );

      // API Routes（認証不要のもの）
      const publicApiPaths = ["/api/plans", "/api/applications", "/api/coupon", "/api/upload-url"];
...
export const { handlers, signIn, signOut, auth } = NextAuth(machinegunAuthConfig);
