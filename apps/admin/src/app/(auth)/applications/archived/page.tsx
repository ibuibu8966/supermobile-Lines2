...
export default function ArchivedApplicationsPage() {
  ...
  // クエリパラメータ生成（常にarchivedOnly=trueを含める）
  const queryParams = useMemo(() => {
    const params: Record<string, string> = { archivedOnly: "true" };
  ...
  const { data: applicationsData, isLoading: isLoadingApplications, error } = useQuery<ApplicationsResponse>({
    queryKey: ["applications", "archived", queryParams],
    queryFn: () => {
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {applications.map((app) => (
                    <tr
                      key={app.id}
                      className="border-b hover:bg-gray-50 bg-gray-100"
                    >
