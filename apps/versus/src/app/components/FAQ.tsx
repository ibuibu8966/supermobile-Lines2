...
export function FAQ() {
  return (
    <section id="faq" className="py-20 bg-card neon-grid">
      <div className="max-w-4xl mx-auto px-4">
        <h2 className="text-3xl font-bold text-center mb-4">
          よくある<span className="text-neon">質問</span>
        </h2>
        ...
        <Accordion type="single" collapsible className="space-y-4">
          {faqItems.map((item, index) => (
            <AccordionItem
              key={index}
              value={`item-${index}`}
              className="bg-background neon-border rounded-lg px-6 neon-box-hover transition-all"
            >
              <AccordionTrigger className="text-left hover:text-neon">
                {item.question}
              </AccordionTrigger>
