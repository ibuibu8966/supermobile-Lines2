...
export class DashboardService {
  ...
  async getDashboard(serviceCode: string): Promise<DashboardResult> {
    ...
    // 本日の申込数
    const todayApplicationCount = await this.prisma.application.count({
      where: {
    ...
    // KYC待ち件数
    const kycPendingCount = await this.prisma.application.count({
      where: {
        serviceId: service.id,
        status: 'SUBMITTED',
      },
    ...
    // 発送待ち件数（入金確認済み）
    const shippingPendingCount = await this.prisma.application.count({
      where: {
        serviceId: service.id,
        status: 'PAID',
      },
    ...
    const activeCustomers = await this.prisma.application.groupBy({
      by: ['customerId'],
      where: {
        serviceId: service.id,
        status: 'COMPLETED',
    ...
    // KYC確認待ち一覧
    const kycPendingList = await this.prisma.application.findMany({
      where: {
        serviceId: service.id,
        status: 'SUBMITTED',
      },
