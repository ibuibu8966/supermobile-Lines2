...
export class ApplicationService {
  ...
  async createCustomerApplication(
    ...
    await uploadFileFn('kyc', idFrontPath, idFrontBuffer, input.idFront.type);
    ...
    await uploadFileFn('kyc', idBackPath, idBackBuffer, input.idBack.type);
    await this.prisma.kycImage.create({
      data: {
        applicationId,
        type: 'ID_BACK',
  ...
  async createAdditionalApplication(
    userId: string,
    input: { planId: string; lineCount: number; couponCode?: string }
    ...
    const application = await this.prisma.$transaction(async (tx) => {
      // Create application (KYC already completed for additional applications)
      const newApplication = await tx.application.create({
        data: {
          ...
          totalAmount,
          status: "SUBMITTED",
          kycStatus: "COMPLETED", // Additional application skips KYC
          couponId,
          couponCode: appliedCouponCode,
        },
      });
